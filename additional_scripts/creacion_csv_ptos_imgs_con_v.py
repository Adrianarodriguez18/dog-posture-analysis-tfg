...
carpeta_txt=r"C:\Users\18adr\OneDrive - Universidade da Coruña\Documentos\Curso 25-26\tfg\imgClasificadas\train_val\labels_filtrados"
# Excel con las imágenes clasificadas, solo vamos a leer que clase es, no si la predicción es correcta o no, por lo que el modelo no afecta.
...
ruta_destino=fr"C:\Users\18adr\OneDrive - Universidade da Coruña\Documentos\Curso 25-26\tfg\imgClasificadas\train_val\resultadosYOLO_large"
...
#     columnas.append(f"{pp}_v [{i}]")
# columnas.append("class")

columnas=["name"]
for i, pp in enumerate(parte_perro):
    columnas.append(f"{pp}_x")
    columnas.append(f"{pp}_y")
    columnas.append(f"{pp}_v")
columnas.append("class")
            ...
            with open(ruta_txt, "r") as f:
                ...
                for i in range(5, len(puntos), 3):
                    if num_punto >= len(parte_perro):
                        ...
                        y=puntos[i+1]
                        v=puntos[i+2]
                        fila[f"{nombre_punto}_x"]=x
                        fila[f"{nombre_punto}_y"]=y
                        fila[f"{nombre_punto}_v"]=v
...
ruta_csv=os.path.join(carpeta_destino, f"puntos_imgs_con_v_2.csv")
